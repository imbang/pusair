...
# =========================================
# obj : convert raw space-delimiter to *.txt.data
...
do
  ...
  do
    nosta=`echo $LINE | awk '{print $1}'`
    nosta=`echo $nosta | sed "s/\"//g"`
    if [ $nosta == "-" ];then continue;fi
    tahun=`echo $LINE | awk '{print $2}'`
    tahun=`echo $tahun | bc`
    bulan=`echo $LINE | awk '{print $3}'`
    bulan=`echo $bulan | bc`
    ...
    for hr in `seq 1 1 $enday`;do
      ...
      #idx=`echo "$hr*2-1" | bc`
      let idx=$idx+$hr
      value=`echo $LINE | awk -v VAR=$idx '{print $VAR}'`
