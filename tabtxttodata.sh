...
# =========================================
# obj : convert raw tab-delimited to *.txt.data
...
do
  ...
  do
    nosta=`echo $LINE | awk '{FS=" \t,;"}{print $1}'`
    nosta=`echo $nosta | sed "s/\"//g"`
    if [ $nosta == "-" ];then continue;fi
    tahun=`echo $LINE | awk '{FS=" \t,;"}{print $2}'`
    tahun=`echo $tahun | bc`
    bulan=`echo $LINE | awk '{FS=" \t,;"}{print $3}'`
    bulan=`echo $bulan | bc`
    ...
    for hr in `seq 1 1 $enday`;do
      ...
      #idx=`echo "$hr*2-1" | bc`
      let idx=$hr+$ofset
      value=`echo $LINE | awk -v VAR=$idx '{FS=" \t,;"}{print $VAR}'`
      #echo $bulan $idx $value
      #if [ $value == "" ];then value=-9999;fi
      if [[ $bulan>=1  && $bulan<=12 ]];then
echo "$nosta $tahun-$bulan-$hr $value" >> $outfil
echo "$nosta $tahun-$bulan-$hr $value"
      fi
    done
  done
done